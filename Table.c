 ...
 * (and may be defined as a macro, which breaks the extern statement).
 * 
 * Fixes to avoid warnings under IRIX.  First arg of XtRealloc() calls
 * needed (unsigned char *) cast.  Argument to XtFree() calls needed (char *).
 * 
 */
...
/* If we want it more motif-like
    { XmNverticalSpacing, XmCSpacing, XtRDimension, sizeof(Dimension),
	XtOffset(TableWidget, table.col_spacing), XtRImmediate, (caddr_t) 0 },
    { XmNhorizontalSpacing, XmCSpacing, XtRDimension, sizeof(Dimension),
	XtOffset(TableWidget, table.row_spacing), XtRImmediate, (caddr_t) 0 },
*/
 ...
 */
{
	...
	if (XtIsManaged(wl[i])) {
		...
		result->locs = (TableLocEntryPtr)
		  XtRealloc((unsigned char *)result->locs,
 ...
 */
{
	...
	switch (result) {
...

	  {
	      ...
	      if (!XtIsWidget(w)) name = XrmQuarkToString(w->core.xrm_name);
	      else name = w->core.name;
